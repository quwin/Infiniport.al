...
def check_guild_conditions(data, player_id, role_requirements, quantity):
    ...
    for item in data:
        ...
        print(role_requirements[1], item.get(role_requirements[1]))
        print(f"Found role for player {player_id} == {player.get("_id")} | {role} == {requirement[1]} | {float(item.get(role_requirements[1], 'inf'))} == {float(quantity)}\n")
